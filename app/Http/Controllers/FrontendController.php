...
class FrontendController extends Controller
{
    ...
    {
        if (isset($request->des_id)) {
            $news = News::latest('id')->where('category', $request->des_id)->where('category','<>','Blog')->where('category','<>','Event')->get();
            $data['des'] = Destination::find($request->des_id);
        } else {
            $news = News::latest('id')->where('category','<>','Blog')->where('category','<>','Event')->get();
        }
        foreach ($news as $new) {
    ...
    public function detail_news($id)
    {
        ...
        $data['nearbys'] = News::where('category', $data['item']->category)->where('id','<>', $id)->get();
        News::find($id)->update(['viewer' => $data['item']->viewer + 1]);
        $data['des'] = Destination::find($data['item']->category);
        $data['destinations'] = Destination::all();
        $data['items'] = News::where('category', 'News')->latest('id')->paginate(3);
    ...
    {
        Carbon::setLocale('vi');
        $data['items'] = News::where('category', 'Blog')->latest('id')->get();
